...
| Firewall / anomaly    | **Review** a paused tool call or **acknowledge** an alert; triage **incidents** under **Firewall** in the main sidebar (see **[Firewall overview](/firewall)**). |
...
| `anomaly_review`  | **Firewall / anomaly**: a suspicious tool call is **paused** until someone approves or rejects it.                               |
| `anomaly_alert`   | **Async detection** alert: informational, **non-blocking**; the person **acknowledges** it.                                   |

Which kinds you actually see depends on your [workspace](/concepts/workspace) plan and features (HITL, AITL, multi-agent, Firewall, and so on).
