...
| **Example YAML** | [Sample manifests](/samples/manifests) — **Telegram** relay (public HTTP + API trigger) and **Gmail** new-mail **HITL** triage (Composio **new message** trigger, label + reply draft). Each declares **inputs** and ships **`phrony.values.yaml`** next to the entry file for CLI **`--values`** / auto-discovery and dashboard apply. |
