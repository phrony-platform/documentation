...
| Action | Where |
| ------ | ----- |
| **Preview wiring** | On an agent’s detail page, under **Version configuration**, open the **Manifest** sub-tab for an interactive diagram of integrations, triggers, and (when applicable) related agents for the snapshot you selected. |
| **Export manifest** | **Agent options** (header menu) on an agent’s detail page, **Export manifest** on the **Manifest** sub-tab, or from the agents list (per agent). Downloads a YAML file (for example `phrony-manifest-<slug>.yaml`). |
| **Apply manifest** | **Import manifest** on the agents list, or **Apply manifest** under **Agent options** on an agent detail page. Upload YAML; the dashboard runs **checks** for placeholders and LLM provider names, then you map **inputs** and can **dry run** or **apply** with the resolved input map. |
| **Example YAML** | [Sample manifests](/samples/manifests) — **Telegram** relay (public HTTP + API trigger) and **Gmail** new-mail **HITL** triage (Composio **new message** trigger, label + reply draft). Each declares **inputs** and ships **`phrony.values.yaml`** next to the entry file for CLI **`--values`** / auto-discovery and dashboard apply. Curated copies also live in the open-source repo [**phrony-platform/manifests**](https://github.com/phrony-platform/manifests). |
...
| `inputs` | Optional declarations for **parameterized** manifests (see [Parameterized values](#manifest-inputs)). |
...
| `anomalyControl` | Optional L1 anomaly behavior (see [Firewall overview](/firewall) and [L1 rules](/firewall/l1-rules)). |
...
## Parameterized values {#manifest-inputs}

You can reuse one manifest across workspaces by leaving **string placeholders** in YAML and supplying concrete values at **lint**, **plan/apply**, or in the **Apply manifest** dialog.

- **Placeholder syntax:** only in **string** values, use `{{inputs.KEY}}` where `KEY` matches `[a-zA-Z0-9_]`. Optional spaces are allowed inside the braces.
- **Declarations:** optional top-level `inputs` lists each key with `key`, `type` (`string`, `llm_provider`, or `integration`), and optional `description` / `default`. When `inputs` is present in a document, every placeholder in that document must be declared there.
- **Dashboard pickers:** for **`llm_provider`** and **`integration`**, the dialog lists workspace resources you can map to each input. When the manifest ties an input to a manifest row whose **`name` is exactly** `{{inputs.KEY}}`, the list is **filtered** to match that row: **`llmProviders[].type`** (for example only **OpenAI**-type providers when the manifest declares `type: openai`), and for integrations the manifest service **`type`** (`http` or `built_in`) and, for built-ins, **`config.toolkitSlug`** when the manifest declares it (for example only Gmail when the manifest expects that toolkit). Plain **`string`** inputs still use a text field.
- **Nothing to pick yet:** if your workspace has no matching LLM provider or integration, the dialog tells you to **create** one (with a shortcut link), then **upload the manifest again** or choose **Check manifest** to refresh—instead of typing a name into a text box.
- **Substitution:** the server and CLI replace placeholders with your map **before** validation. Unknown `{{…}}` patterns in strings are rejected.
- **Workspace LLM names:** agents still reference tenant LLM providers by **name**. If an export used names that do not exist in the target workspace, use a **values** map and/or the dashboard mapping step so names resolve to real providers (APIs accept an optional `llmProviderOverrides` map from manifest name → workspace provider name).

**Values file (CLI):** beside your manifest entry (or via `--values` / `PHRONY_MANIFEST_VALUES`), a `phrony.values.yaml` file can hold:
...
**In the Phrony dashboard:** the **Import manifest** and **Apply manifest** dialogs collect the same **inputs** map (including **`llm_provider`** and **`integration`** pickers) instead of reading `phrony.values.yaml`; you still upload the manifest YAML first.

**Preflight:** the API can run a non-mutating check that returns structured **blockers** (for example missing LLM provider names or unresolved placeholders) before you dry run or apply.
...
| Action | **Phrony dashboard** | **CLI** |
| --- | --- | --- |
| Dry run | **Dry run** in **Import manifest** or **Apply manifest** after **checks** pass | **`phrony plan`** |
| Apply | **Apply** in the same dialog | **`phrony apply`** (runs plan first, then prompts unless **`--auto-approve`**) |
