...
| **Apply manifest** | **Import manifest** on the agents list, or **Apply manifest** under **Agent options** on an agent detail page. Upload YAML; the dashboard runs **checks** for placeholders and LLM provider names, then you can **dry run** or **apply** with the resolved input map. |
...
- **Declarations:** optional top-level `inputs` lists each key with `key`, `type` (`string`, `llm_provider`, or `integration`), and optional `description` / `default`. When `inputs` is present in a document, every placeholder in that document must be declared there. In the **Apply manifest** dialog, **`llm_provider`** shows your existing LLM providers, and **`integration`** shows your existing integrations—so you pick a row by name instead of guessing text. Each type includes a link to create a new resource if the list is empty.
