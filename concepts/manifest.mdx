...
| **Example YAML** | [Sample manifests](/samples/manifests) — **Telegram** relay (public HTTP + API trigger) and **Gmail** new-mail **HITL** triage (Composio **new message** trigger, label + reply draft); each uses **inputs** and **`phrony.values.yaml`** next to the entry YAML in this documentation repository. |
