...
For **why** Firewall exists and **dashboard** workflows, start with **[Firewall overview](/firewall)**.
...
2. **Follow-up correlation** — The platform **replays** the same rule definitions against recorded events so **per-run / per-session counters** stay consistent and **Firewall incidents**, alerts, and audits reflect the run. That path does **not** undo an inline decision after a call has already finished.
...
**`call_count_in_window_gt`:** Enforced in **async / Firewall-side** evaluation. The **inline** checker at attempt time does **not** apply this predicate by itself. Use it for **follow-up** detection (incidents, alerts), not a guarantee at every single attempt.
...
### Email or messaging — burst detection (async / Firewall)
...
Remember: **`call_count_in_window_gt`** is evaluated on the **async / Firewall** path. Pair it with how you want operators to respond (incidents, **`anomaly_alert`** tasks, and so on). A rule whose **`when`** is **only** a window predicate does **not** drive **inline** checks by itself—use **`call_count_in_run_gt`** or argument predicates when you need a hard gate on every attempt.
