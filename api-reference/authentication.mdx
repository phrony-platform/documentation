...
You create **API keys** in the **Phrony dashboard** under **Settings** (or your workspace settings, depending on layout). Each key is a single secret; issued keys are prefixed with **`phk_`**.

You must not commit keys to source control or embed them in client-side code that end users can read. Use environment variables or a secret manager on your server.
...
Every authenticated request adds the same header. Replace `RUN_ID` after you [start a run](/api-reference/agents-and-runs#start-a-run).
